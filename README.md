...
- 苦手ローマ字特訓：学習履歴から苦手を抽出し、味方と時間無制限で反復練習
...
- 全180体のモンスター（モード別グループ、5体ごとに進化系、10体ごとにボス）
- 全10背景からプレイごとにランダム選択
- 現在と次のモンスターだけを先読みする軽量画像ロード
- 失敗問題を3〜5問後に再出題する復習キュー
- ヒント後に自力成功したときの `RETRY CLEAR!`
- 苦手候補を優先し、初回は表示・2周目から非表示にする時間無制限の特訓
- 特訓中は3回のミスタイプで全文を表示し、いつでもその場で終了可能
...
src/
  ...
  monster-data.js  180体の分類・グループデータ
